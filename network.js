...
var Graph, Link, NetworkGadget, Node, calc_r, calc_width, envelope_handler, export_click, getBareJidFromJid, getDomainFromJid, graph, graph_brake, graph_brake_id, handleFileSelect, hexToRgb, hide_exim, init_ui, jg, jsnx_start, line_label, line_slider, link_nodes, log_handler, max_r, max_width, min_fontsize, min_r, min_width, node_label, node_slider, reset_graph, show_exim, start_graph, to_jsnx,
  __hasProp = {}.hasOwnProperty,
  __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
  __indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; },
  __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; };

NetworkGadget = (function(_super) {
  __extends(NetworkGadget, _super);

  function NetworkGadget(callback) {
    var gadgets;
    gadgets = ["SelectorGadget", "StatGadget"];
    NetworkGadget.__super__.constructor.call(this, callback, "NetworkGadget", gadgets);
  }

  NetworkGadget.prototype.init = function() {
    var env;
    env = new Envelope("enter");
    return env.publish();
  };

  NetworkGadget.prototype.openAppCallback = function(envelope, message) {
    if (/StatGadget.xml$/.test(envelope.sender) || /SelectorGadget.xml$/.test(envelope.sender) || /NetworkGadget.xml$/.test(envelope.sender)) {
  ...
  return NetworkGadget;

})(MultiGadget);

window.gadget_init = function() {
  var gadget;
  gadget = new NetworkGadget(function(envelope, message, gadget) {
...
Graph = (function() {
  ...
  Graph.prototype.export_gexf = function() {
    ...
    for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
      ...
      tempStore[n.id] = i;
      color = hexToRgb(this.color(getDomainFromJid(n.id)));
  ...
  Graph.prototype.create_svg_elements = function() {
    ...
    this.circles.select("text").attr("font-size", function(d) {
      return "" + (d3.max([min_fontsize, d.r])) + "px";
    ...
    node.append("svg:circle").attr("r", (function(_this) {
      ...
      return function(d) {
        return _this.color(getDomainFromJid(d.id));
      };
  ...
  Graph.prototype.get_line_width = function(d) {
    return d.width = ((d.count / this.mode.max_line_count) * calc_width) + min_width;
  };

  Graph.prototype.get_node_width = function(d) {
    return d.r = ((d.count / this.mode.max_node_count) * calc_r) + min_r;
  };
  ...
  Graph.prototype.add_physical = function(log) {
    ...
    if (from !== from_domain) {
      if (!ns[from_domain]) {
        new_fromD = new Node(from_domain, void 0, timestamp);
    ...
    if (to !== to_domain) {
      if (!ns[to_domain]) {
        new_toD = new Node(to_domain, void 0, timestamp);
    ...
    if (fdNode && tdNode) {
      ...
      return link_nodes(tdNode, tNode, p);
    } else if (fdNode) {
      link_nodes(fNode, fdNode, p);
      return link_nodes(fdNode, tNode, p);
    } else if (tdNode) {
      link_nodes(fNode, tdNode, p);
      return link_nodes(tdNode, tNode, p);
    } else {
      return link_nodes(fNode, tNode, p);
    }
  };
  ...
  Graph.prototype.add_logical = function(log) {
    ...
    return link_nodes(fNode, tNode, l, log.tags.p2p);
  };
  ...
  Graph.prototype.add_nodes = function(from, to, mode, timestamp) {
    ...
    n = mode.nodes;
    if (!ns[from]) {
      new_from = new Node(from, void 0, timestamp);
      n.push(ns[from] = new_from);
    }
    if (!ns[to]) {
      new_to = new Node(to, void 0, timestamp);
      n.push(ns[to] = new_to);
    }
    fNode = ns[from];
    tNode = ns[to];
...
reset_graph = function() {
  ...
  graph.force.stop();
  _ref = [graph.physical, graph.logical];
  for (_i = 0, _len = _ref.length; _i < _len; _i++) {
    obj = _ref[_i];
    ...
    _ref2 = graph.physical.nodes_obj;
    for (value = _k = 0, _len2 = _ref2.length; _k < _len2; value = ++_k) {
      key = _ref2[value];
      delete obj.nodes_obj[key];
    }
  }
  graph.force.stop().nodes(graph.mode.nodes).links(graph.mode.links);
  nd = new Date();
  ...
  graph.logs = [];
  return graph.limit((graph.node_thresh = Infinity), (graph.line_thresh = Infinity));
};
...
start_graph = function() {
  ...
  line_slider = d3.select("input#line_thresh_slider").property("disabled", false).attr("max", 51).attr("value", 51);
  line_label = d3.select("label#line_limit_label");
  node_slider = d3.select("input#node_thresh_slider").property("disabled", false).attr("max", 51).attr("value", 51);
...
jsnx_start = function() {
  ...
  return jsnx.draw(jg, {
    ...
    node_style: {
      "fill": function(d) {
        return color(getDomainFromJid(d.node));
      },
      "cursor": "pointer",
      "stroke": "#000",
      "stroke-width": "1.5px"
...
handleFileSelect = function(evt) {
  ...
  reader.onload = (function(theFile) {
    return function(e) {
      return graph.import_logs(e.target.result);
    };
  })(f);
...
init_ui = function() {
  ...
  d3.select("select#viz_select").on("change", function() {
    switch (this.value) {
      case "viz_logical":
        return graph.show_logical();
      case "viz_physical":
        return graph.show_physical();
    }
  });
  d3.select("input#node_thresh_slider").on("change", function() {
    console.log(this.value);
    graph.limit(this.value, graph.line_thresh);
    return node_label.text("Node Limit: " + (this.value === this.max && "All" || this.value));
  });
  d3.select("input#line_thresh_slider").on("change", function() {
    console.log(this.value);
    graph.limit(graph.node_thresh, this.value);
    return line_label.text("Link Limit: " + (this.value === this.max && "All" || this.value));
  });
  ...
  d3.select("#gexf_button").on("click", function() {
    return graph.export_gexf();
  });
  d3.select("#json_export").on("click", function() {
    return graph.export_logs();
  });
  d3.select("#imex_button").on("click", show_exim);
  return document.getElementById('file').addEventListener('change', handleFileSelect, false);
};
